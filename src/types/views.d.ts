...
declare type RowUserType<T = any> = {
	...
	remark: string;
	roles: T[];
	sysUserGradeGroupResponses: T[];
	sysUserPostDeptResponses: T[];
	unitId: string;
	unitName: string;
	userId: number;
	userName: string;
...
// unit
declare type RowUnitType<T = any> = {
	deptId: number;
	deptName: string;
	image: string;
	path: string;
...
interface SysUnitTableType extends TableType {
	data: RowUnitType[];
}

declare interface SysUnitrState {
	tableData: SysUnitTableType;
}
